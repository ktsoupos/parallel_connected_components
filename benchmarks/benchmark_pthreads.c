...
int run_pthreads_benchmarks(const Graph* g, int num_threads) {
    ...
    /* Small delay before benchmarks */
    printf("\nWaiting 1 second before parallel benchmarks...\n");
    sleep(5);
    ...
    printf("Label Propagation (Sync) completed in %.5f seconds\n", elapsed_lp);
    cc_result_print_stats(result_lp, g);

    /* Delay between benchmarks */
    sleep(5);

    /* Run Label Propagation Async with work-stealing */
    printf("\n=== Parallel Label Propagation (Async Work-Stealing) ===\n");
    struct timespec start_lp_async, end_lp_async;
    clock_gettime(CLOCK_MONOTONIC, &start_lp_async);
    CCResult* result_lp_async = label_propagation_async_pthreads(g, num_threads);
    clock_gettime(CLOCK_MONOTONIC, &end_lp_async);

    if (result_lp_async == NULL) {
        fprintf(stderr, "Error: Async label propagation failed\n");
        cc_result_destroy(result_seq);
        cc_result_destroy(result_static);
        cc_result_destroy(result_dynamic);
    ...
    const double elapsed_lp_async = (end_lp_async.tv_sec - start_lp_async.tv_sec) +
                                    (end_lp_async.tv_nsec - start_lp_async.tv_nsec) / 1e9;
    printf("Label Propagation (Async) completed in %.5f seconds\n", elapsed_lp_async);
    cc_result_print_stats(result_lp_async, g);
    ...
    bool lp_async_correct = (result_seq->num_components == result_lp_async->num_components);

    if (static_correct && dynamic_correct && lp_correct && lp_async_correct) {
        printf("✓ All algorithms found %d components\n", result_seq->num_components);
    } else {
        printf("✗ WARNING: Component counts DIFFER\n");
        printf("  Sequential:            %d components\n", result_seq->num_components);
        ...
        printf("  Label Prop Sync:       %d components %s\n", result_lp->num_components, lp_correct ? "✓" : "✗");
        printf("  Label Prop Async:      %d components %s\n", result_lp_async->num_components, lp_async_correct ? "✓" : "✗");
    }
    ...
    printf("Afforest Dynamic  (%d threads):            %.5f seconds (%d iterations)\n",
           num_threads, elapsed_dynamic, result_dynamic->num_iterations);
    printf("Label Prop Sync   (%d threads):            %.5f seconds (%d iterations)\n",
           num_threads, elapsed_lp, result_lp->num_iterations);
    printf("Label Prop Async  (%d threads, WorkSteal): %.5f seconds (%d iterations)\n",
           num_threads, elapsed_lp_async, result_lp_async->num_iterations);

    /* Compute speedups for all parallel versions */
    if (elapsed_seq > 0.0) {
        const double speedup_lp = elapsed_seq / elapsed_lp;
        const double speedup_lp_async = elapsed_seq / elapsed_lp_async;
        const double efficiency_lp = speedup_lp / (double)num_threads * 100.0;
        const double efficiency_lp_async = speedup_lp_async / (double)num_threads * 100.0;

        printf("\n=== Speedups vs Sequential ===\n");
        printf("Afforest Static:     %.2fx (%.1f%% efficiency)\n", speedup_static, efficiency_static);
        printf("Afforest Dynamic:    %.2fx (%.1f%% efficiency)\n", speedup_dynamic, efficiency_dynamic);
        printf("Label Prop Sync:     %.2fx (%.1f%% efficiency)\n", speedup_lp, efficiency_lp);
        printf("Label Prop Async:    %.2fx (%.1f%% efficiency)\n", speedup_lp_async, efficiency_lp_async);

        /* Find fastest algorithm */
        printf("\n=== Algorithm Ranking ===\n");
        double times[] = {elapsed_static, elapsed_dynamic, elapsed_lp, elapsed_lp_async};
        const char* names[] = {"Afforest Static", "Afforest Dynamic", "Label Prop Sync", "Label Prop Async"};
    ...
    cc_result_destroy(result_dynamic);
    cc_result_destroy(result_lp);
    cc_result_destroy(result_lp_async);
